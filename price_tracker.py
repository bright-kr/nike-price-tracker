"""
Nike Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Nike.

Docs  : https://docs.brightdata.co.kr/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.co.kr/products/insights/price-tracker/nike
"""
...
# Search for "Nike" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
...
if not DATASET_ID:
    raise EnvironmentError(
        ...
        "2. Search for 'Nike' and copy the Web Scraper ID.\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Nike product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Nike products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Nike category page.

    Example:
        results = discover_by_category("https://nike.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.nike.com/t/sample-shoe-12345/ABCD1234",
        # Add more Nike product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "nike_prices.json"
    ...
    # cat_results = discover_by_category("https://nike.com/category/sample", limit=50)
